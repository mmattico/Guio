...
class LoginPassword extends StatefulWidget {
  final String usuarioOEmail; // Agregamos el campo

  const LoginPassword({
    super.key,
    required this.usuarioOEmail, // Marcamos como required
  });
...
class _LoginPasswordState extends State<LoginPassword> {
        ...
        RichText(
          ...
          text: TextSpan(
            children: [
              ...
              TextSpan(
                text: "${widget.usuarioOEmail} ya fue utilizado para acceder. Ingresa tu contraseña para seguir navegando.", // Segundo texto
                style: TextStyle(
                  fontSize: 18,
    ...
    return SizedBox(
      ...
      child: ElevatedButton(
        onPressed: _isValidatingInfo? null : () async {
          ...
          try {
            if(RegExp(r'^[^@]+@[^@]+\.[^@]+').hasMatch(widget.usuarioOEmail)) {
              await _getUserByEmail(widget.usuarioOEmail);
            } else {
              await _getUserByUsername(widget.usuarioOEmail);
            }
          } finally {
            setState(() {
  ...
  _signup(context) {
    return Column(
      ...
      children: [
        const Text("¿Registrarse con otro correo?"),
        TextButton(
            onPressed: () {
